...
def main(study_networks):
    ignore_txt = MRI_ROOT / 'codes/data_to_ignore.txt'
    for study_network in study_networks:
        PHOENIX_DIR = data_flow_dir / study_network / 'PHOENIX'
        for i in Path(PHOENIX_DIR / 'PROTECTED').glob(
            '*/raw/*/mri/*_MR_*_[1234].[zZ][iI][pP]'):
            filepath = str(i)

            data_to_ignore_file = ignore_txt
            if filepath in data_to_ignore_file.read_text().splitlines():
                continue

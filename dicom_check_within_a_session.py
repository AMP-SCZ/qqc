...
def parse_args(argv):
    ...
    parser = argparse.ArgumentParser(
            description='Check dicom files from a scan session')

    # image input related options
    parser.add_argument(
            '--json_files', type=str, nargs='+',
            help='List of json files created from dcm2niix, from a single '
                 'scan session.')

    parser.add_argument(
            '--nifti_prefixes', type=str, nargs='+',
            help='List of nifti image prefixes to catch relevant json files.')

    parser.add_argument(
            '--multi_file_dir', type=str,
            help='Path of a directory, where there are unique json or dicom '
                 'files to be compared to each other.')

    parser.add_argument(
            '--print_diff', action='store_true',
            help='Print different items between the json_files')

    parser.add_argument(
            '--print_shared', action='store_true',
...
def json_check_from_a_single_session(json_files: List[str],
               print_diff: bool = True,
               print_shared: bool = False):
    '''Iteratively compare between MR protocol json files and print outputs

    This function expects json files from a single scan session, where the
    whole session has not been interfered.
    '''
    dicts = []
    for i in json_files:
        with open(i, 'r') as f:
    ...
    for single_dict in dicts:
        print((x,y) for x, y in single_dict
                                if x in ['ImageOrientationPatientDICOM', 'ShimSetting'])
        print(dict((x,y) for x, y in single_dict
                                if x in ['ImageOrientationPatientDICOM', 'ShimSetting']))
        single_dict = dict((x,y) for x, y in single_dict
                if x in ['ImageOrientationPatientDICOM', 'ShimSetting'])
...
if __name__ == '__main__':
    ...
    df_all_diff, df_all_shared = json_check_from_a_single_session(
